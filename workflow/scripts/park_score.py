...
# Usage: python park_score.py stats.tsv quality.tsv out.tsv --mag MAG1
...
Completeness: checkm2 column 2
Contamination: checkm2 column 3
...
def parse_checkm(quality_tsv: Path, mag_name: str) -> tuple[float, float]:
    """
...
def main(stats_tsv: Path, quality_tsv: Path, out_tsv: Path, mag_name: str) -> None:
    num_contigs, amb_bases = parse_seqkit(stats_tsv)
    comp, cont = parse_checkm(quality_tsv, mag_name)
...
if __name__ == "__main__":
    ...
    main(args.stats_tsv, args.quality_tsv, args.out_tsv, args.mag)
