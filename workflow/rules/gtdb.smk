...
rule gtdbtk_classify:
    ...
    gtdbtk classify_wf --genome_dir {INPUT_DIR} --out_dir {GTDB_DIR} --prefix run --cpus {threads} --data_dir {input.db} || true
        # Extract lineage for this MAG
        echo -e "lineage" > {output.tsv}
        if [ -s {GTDB_DIR}/run.bac120.summary.tsv ]; then
            awk -F '\t' -v s={Path(input.mag).name} '$1==s{print $2}' {GTDB_DIR}/run.bac120.summary.tsv >> {output.tsv}
        elif [ -s {GTDB_DIR}/run.ar53.summary.tsv ]; then
            awk -F '\t' -v s={Path(input.mag).name} '$1==s{print $2}' {GTDB_DIR}/run.ar53.summary.tsv >> {output.tsv}
        else
            echo "" >> {output.tsv}
        fi
        """
